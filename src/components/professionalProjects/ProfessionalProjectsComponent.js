      ...
      <div className='w3-container w3-card w3-white w3-margin-bottom'>
        ...
        {repositories.map((repo, index) => {
          ...
          <div className='w3-container' key={`${repo.Id}(${index})`}>
            ...
            <span style={{backgroundColor: '#009688', color: '#ffffff', padding: 2}}>{repo.language}</span>
            <hr />
          </div>
        )})}
      </div>
      <div className='w3-container w3-card w3-white'>
        ...
        <div className='w3-container' key={`${learning.Id}(${index})`}>
          ...
          {learning.tags.map((tag, index) => {
            return (
              <span style={{backgroundColor: '#009688', color: '#ffffff', padding: 2, margin: 2}} key={`(${index})`}>{tag}</span>
            )
          })}
          <hr />
        </div>
